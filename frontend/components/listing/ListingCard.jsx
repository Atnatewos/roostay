...
// Uses Cloudinary thumbnail transformations for optimized loading
'use client';
...
export default function ListingCard({ listing, showFavorite = true }) {
  ...
  const optimizedImageUrl = listing.primaryImage ? getThumbnailUrl(listing.primaryImage) : PLACEHOLDER;

  return (
    <div className="listing-card">
      <Link href={'/listings/' + listing.id} className="listing-card__link">
        <div className="listing-card__image-wrapper">
          <img src={optimizedImageUrl} alt={listing.title} className="listing-card__image" loading="lazy" onError={(e) => { e.target.src = PLACEHOLDER; }} />
          ...
          {showFavorite && (
            <button className="listing-card__favorite" onClick={(e) => handleFavoriteToggle(e, listing.id)} aria-label="Toggle favorite">
              <svg className="listing-card__favorite-icon" viewBox="0 0 24 24" width="24" height="24" fill="none" stroke="currentColor" strokeWidth="2">
        ...
        <div className="listing-card__content">
          <div className="listing-card__header"><h3 className="listing-card__title">{listing.title}</h3></div>
          <p className="listing-card__location">{listing.city || listing.location?.city}</p>
          <div className="listing-card__details">
            <span>{listing.bedrooms} bed{listing.bedrooms !== 1 ? 's' : ''}</span>
            <span className="listing-card__detail-separator">·</span>
