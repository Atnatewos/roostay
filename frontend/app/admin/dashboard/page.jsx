...
// All labels and content are config-driven via useConfig()
...
export default function AdminDashboardPage() {
  ...
  // Load dashboard content strings from config
  const dashboardContent = content?.admin?.dashboard || content?.dashboard || {};
    ...
    async function fetchDashboardStats() {
      try {
        ...
        if (err instanceof ApiError) {
          if (err.status === 401) setError('Please log in as an administrator.');
          else if (err.status === 403) setError('You do not have admin privileges.');
    ...
    return (
      <div className="container" style={{ paddingTop: '3rem', paddingBottom: '4rem', textAlign: 'center' }}>
        <h1 style={{ marginBottom: '1rem' }}>
          {dashboardContent.title || 'Admin Dashboard'}
        </h1>
    ...
    <div className="container" style={{ paddingTop: '3rem', paddingBottom: '4rem' }}>
      ...
      <div style={{ marginBottom: '2.5rem' }}>
        ...
        <p style={{ color: 'var(--color-text-secondary)' }}>
          {dashboardContent.subtitle || 'Platform overview and system management.'}
        </p>
      </div>
      ...
      <div style={{ display: 'flex', gap: '0.75rem', marginBottom: '2.5rem', flexWrap: 'wrap' }}>
        <Link href={constants.ROUTES.ADMIN_USERS} className="btn btn--outline btn--sm">
          {dashboardContent.manageUsers || 'Manage Users'}
        </Link>
        <Link href={constants.ROUTES.ADMIN_LISTINGS} className="btn btn--outline btn--sm">
          {dashboardContent.moderateListings || 'Moderate Listings'}
        </Link>
        <Link href={constants.ROUTES.ADMIN_PAYMENTS} className="btn btn--outline btn--sm">
          {dashboardContent.verifyPayments || 'Verify Payments'}
        </Link>
        <Link href={constants.ROUTES.ADMIN_WITHDRAWALS} className="btn btn--outline btn--sm">
          {dashboardContent.processWithdrawals || 'Process Withdrawals'}
        </Link>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: '1rem', marginBottom: '2.5rem' }}>
        <Card padding="lg" hoverable>
          <p style={{ fontSize: 'var(--font-size-xs)', textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--color-text-light)', marginBottom: '0.5rem' }}>
            {dashboardContent.totalUsers || 'Total Users'}
          </p>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: '1rem', marginBottom: '2.5rem' }}>
        <Card padding="lg" hoverable>
          <p style={{ fontSize: 'var(--font-size-xs)', textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--color-text-light)', marginBottom: '0.5rem' }}>
            {dashboardContent.totalListings || 'Total Listings'}
          </p>
        ...
        <Card padding="lg" hoverable style={{ borderLeft: stats.pendingApproval > 0 ? '3px solid var(--color-warning)' : 'none' }}>
          <p style={{ fontSize: 'var(--font-size-xs)', textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--color-text-light)', marginBottom: '0.5rem' }}>
            {dashboardContent.pendingApproval || 'Pending Approval'}
          </p>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: '1rem', marginBottom: '2.5rem' }}>
        <Card padding="lg" hoverable>
          <p style={{ fontSize: 'var(--font-size-xs)', textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--color-text-light)', marginBottom: '0.5rem' }}>
            {dashboardContent.totalBookings || 'Total Bookings'}
          </p>
      ...
      <h2 style={{ fontSize: 'var(--font-size-xl)', fontWeight: 'var(--font-weight-semibold)', marginBottom: '1rem' }}>
        {dashboardContent.revenue || 'Revenue & Payments'}
      </h2>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: '1rem', marginBottom: '2.5rem' }}>
        <Card padding="lg" hoverable>
          <p style={{ fontSize: 'var(--font-size-xs)', textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--color-text-light)', marginBottom: '0.5rem' }}>
            {dashboardContent.totalRevenue || 'Total Revenue'}
          </p>
        ...
        <Card padding="lg" hoverable>
          <p style={{ fontSize: 'var(--font-size-xs)', textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--color-text-light)', marginBottom: '0.5rem' }}>
            {dashboardContent.serviceFees || 'Service Fees'}
          </p>
        ...
        <Card padding="lg" hoverable style={{ borderLeft: stats.pendingPayments > 0 ? '3px solid var(--color-warning)' : 'none' }}>
          <p style={{ fontSize: 'var(--font-size-xs)', textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--color-text-light)', marginBottom: '0.5rem' }}>
            {dashboardContent.pendingPayments || 'Pending Payments'}
          </p>
        ...
        <Card padding="lg" hoverable style={{ borderLeft: stats.pendingWithdrawals > 0 ? '3px solid var(--color-warning)' : 'none' }}>
          <p style={{ fontSize: 'var(--font-size-xs)', textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--color-text-light)', marginBottom: '0.5rem' }}>
            {dashboardContent.pendingWithdrawals || 'Pending Withdrawals'}
          </p>
