...
// Validates ID details, hosting experience, and agreement data
const Joi = require('joi');

const hostApplicationValidator = {
  /**
   * Schema for submitting a host application.
   * Ensures all required identity and experience fields are present and valid.
   */
  apply: Joi.object({
    idType: Joi.string()
      ...
      .required()
      .messages({
        'any.only': 'Invalid ID type. Must be kebele_id, passport, drivers_license, or national_id.',
    ...
    idFrontImageUrl: Joi.string()
      ...
      .required()
      .messages({
        'string.uri': 'ID front image must be a valid URL.',
        'any.required': 'ID front image is required.',
      }),
    idBackImageUrl: Joi.string()
      ...
      .allow(null, '')
      .messages({
        'string.uri': 'ID back image must be a valid URL.',
      }),
    ...
    propertyCount: Joi.string()
      ...
      .required()
      .messages({
        'any.only': 'Property count must be 1-2, 3-5, or 5+.',
