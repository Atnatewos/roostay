...
const userService = {
   ...
   * @throws {ValidationError} If registration is disabled or data is invalid
  ...
  async register(userData) {
    ...
    // Hash the password with configured salt rounds
  ...
  async login(email, password) {
    const user = await queryOne(
      `SELECT id, email, phone_number, first_name, last_name, password_hash,
              role, is_verified, is_active, login_attempts, locked_until
    ...
    if (user.locked_until && new Date(user.locked_until) > new Date()) {
      ...
      throw new AuthError(
        `Account temporarily locked due to multiple failed attempts. Please try again in ${minutesLeft} minute(s).`
      );
    }
    ...
    if (!isPasswordValid) {
      ...
      if (attempts >= maxAttempts) {
        ...
        throw new AuthError(
          `Account locked after ${maxAttempts} failed attempts. Please try again in ${lockoutMinutes} minutes.`
        );
      }
   ...
   * Issues a new access token without requiring password re-entry.
   *
   * @param {string} refreshToken - The refresh token
   * @returns {Promise<Object>} New token pair
   * @throws {AuthError} If the refresh token is invalid
   */
  async refreshToken(refreshToken) {
    const decoded = verifyRefreshToken(refreshToken);

    // Fetch the user to verify they still exist and are active
    const user = await queryOne(
   ...
   * Only updates provided fields; null/undefined fields are ignored.
   *
   * @param {string} userId - The user ID
   ...
   * Supports filtering by role, verification status, and active status.
   *
